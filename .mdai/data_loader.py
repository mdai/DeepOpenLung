    ...
    def __init__(self, split, config, phase="train", split_comber=None):
        ...
        if self.channel == 2:
            ...
            if "cleanimg" in config and config["cleanimg"]:
                self.filenames = [
                    os.path.join(data_dir, "%s_clean.npy" % idx) for idx in idcs
                ]
            else:
                self.filenames = [
                    os.path.join(data_dir, "%s_img.npy" % idx) for idx in idcs
                ]
        self.kagglenames = [
            f for f in self.filenames if len(f.split("/")[-1].split("_")[0]) > 20
        ]
        self.lunanames = [
            f for f in self.filenames if len(f.split("/")[-1].split("_")[0]) < 20
        ]
        ...
        for bbox in bboxes:
            for i, anchor in enumerate(anchors):
                ...
                label[:, :, :, i, 0] = binary_dilation(
                    label[:, :, :, i, 0].astype("bool"), structure=struct, iterations=1
                ).astype("float32")
